 ...
 */
public final class ColumnDefinition<T extends Object> implements Comparable<ColumnDefinition>, ColumnDefinitionModifier {
    ...
    ColumnDefinition(Field field, ExcelColumn excelColumn, TabulationBeanConfiguration<T> tableConfiguration) {
        ...
        this.theadStyle = tableConfiguration.getTheadStyleMap().get(excelColumn.theadStyleIndex());
        this.tbodyStyle = tableConfiguration.getTbodyStyleMap().get(excelColumn.tbodyStyleIndex());
        Annotation annotation = ReflectUtil.getFirstMarkedAnnotation(field, DataValid.class);
        if (annotation != null) {
            this.dvBuilder = AbstractDvBuilder.getInstance(annotation);
        }
        try {
            this.cellsWriter = excelColumn.cellsWriter().getDeclaredConstructor().newInstance();
        } catch (Exception e) {
            throw new IllegalColumnConfigureException("ColWriter lack of parameterless constructors! field : " + fieldName);
        }
        this.cellsWriter.supportedDataType(field);
    }

    public TabulationBeanConfiguration<T> getTableConfiguration() {
